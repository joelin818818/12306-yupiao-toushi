...
在 12306 余票查询页增强展示：**车次局属、车型、席位图鉴、票价折扣**（左侧独立信息面板 + 悬停行高亮）。所有功能默认开启，无设置面板。
...
| 功能 | 说明 | 数据来源 |
| --- | --- | --- |
| 局属 | 左侧信息面板的「局属」行显示路局（如「成都局」）；悬停浮窗内显示完整路局名（如「中国铁路成都局集团有限公司」） | 12306 官方接口 |
| 车型 | 左侧信息面板「车型」行显示官方车型（如 `CR400BF-S`，蓝色）+ 中文类别（和谐号/复兴号）；无数据显示灰色 `—` | 12306 官方接口 |
| 席位图鉴 | 悬停车次行，右侧停靠浮窗展示各席别座位实拍图（多图完整展示，不滚动截断） | 12306 官方接口 |
| 票价 / 折扣 | 各席别余票格下方显示票价并标注折扣（如 `¥553 8.5`） | 12306 页面解析 |
| 行高亮 | 悬停车次行时整行淡蓝高亮，便于与左侧面板对照 | 本地样式 |
...
src/
  ...
  ui.js      界面渲染：右侧停靠浮窗、左侧车次信息面板（局属+车型）、票价角标、行高亮
